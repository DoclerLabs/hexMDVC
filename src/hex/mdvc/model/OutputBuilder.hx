...
class OutputBuilder
{
	...
	macro static public function build() : Array<Field> 
	{
		...
		for ( f in fields )
		{
			var isOutput = f.meta.filter( function ( m ) { return m.name== OutputBuilder.OutputAnnotation; } ).length > 0;
			if ( isOutput ) 
			{
				switch( f.kind )
				{ 
	...
	{
		var outputDefinition 	= OutputBuilder._getOutputDefinition( f );
		var e 					= OutputBuilder._buildClass( outputDefinition );
	...
	static function _getOutputDefinition( f ) : { name: String, pack: Array<String>, fullyQualifiedName: String }
	{
		...
		switch ( f.kind )
		{
			case FVar( TPath( p ), e ):
				...
				OutputBuilder._checkIOutputImplementation( f, p );
				connectionDefinition = OutputBuilder._getConnectionDefinition( p.params );
	...
	{
		var className 	= "__" + OutputBuilder.OutputAnnotation + '_Class_For__' + interfaceName.name;
